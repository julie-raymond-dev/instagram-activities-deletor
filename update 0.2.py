...
"""
    ...
    while (mode := input("Choose mode [1/2/3/4]: ").strip()) not in ["1", "2", "3", "4"]:
        pass
    MODE = int(mode)
                ...
                for el in driver.find_elements(By.CSS_SELECTOR, 'span[data-bloks-name="bk.components.Text"]'):
                    if el.text == "Select":
                        ...
                        if any(j.text == "No results" for j in driver.find_elements(By.CSS_SELECTOR, 'span[data-bloks-name="bk.components.Text"]')):
                            logging.info("No items found. DONE. Quitting")
                            driver.quit()
                            sys.exit(0)
                ...
                for icon in icons:
                    ...
                    if style.startswith('mask-image: url("https://i.instagram.com/static/images/bloks/icons/generated/circle__outline'):
                        ...
                        if selected_count >= AT_ONCE_DELETE:
                            break
            if selected_count == 0:
                logging.info("No selectable items found. DONE. Quitting")
                driver.quit()
                sys.exit(0)
